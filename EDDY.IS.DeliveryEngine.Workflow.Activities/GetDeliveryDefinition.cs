...
namespace EDDY.IS.DeliveryEngine.Workflow.Activities
{
    public sealed class GetDeliveryDefinition : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            if (logDebug)
                DeliveryEngineDataService.DeliveryEngineDAO.TempLog(leadData != null ? leadData.LeadId : 0, 10, 0,
                    "Retrieving Delivery Definition for LeadId: " + leadData.LeadId + " CRID: " + leadData.CRId, 1, false);            

            //Get All Defs for CSR
            List<DeliveryDefinition> defs = DeliveryEngineDataService.DeliveryEngineDAO.GetCRDeliveryDefinitionsForLead(leadData.CRId, leadData.LeadId);
            ...
            if (definitionToUse == null)
            {
                if (logDebug)
                    DeliveryEngineDataService.DeliveryEngineDAO.TempLog(leadData != null ? leadData.LeadId : 0, 10, 0,
                        "Failed to find Delivery Definition for LeadId: " + leadData.LeadId + " CRID: " + leadData.CRId, 1, false);
            }
            else
            {
                //Add Endpoints to complete definition
                definitionToUse.EndPoints = DeliveryEngineDataService.DeliveryEngineDAO.GetDefinitionEndpoints(definitionToUse);
            }
